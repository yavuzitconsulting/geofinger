...
namespace GeoFingerApi.Services
{
    public class InMemoryFingerDataStore : IFingerDataStore
    {
        ...
        //    string[] data = this.messages.Where(x =>
        //    (x.lat >= (lat - (decimal)0.0003)) &&
        //    (x.lat <= (lat + (decimal)0.0003)) &&
        //    (x.lon >= (lon - (decimal)0.0003)) &&
        //    (x.lon <= (lon + (decimal)0.0003))).Select(x => "\"" + x.message + "\"" + " @ " + x.timestamp).ToArray();
        //    foreach (string s in data)
        //    {
        //        Console.WriteLine($"Following messages was found between these values: {s}");
        //    }
        //    return data;
        //}
